...
internal static class Benchmark
{
    public static int Run(string[] args)
    {
        ...
        try
        {
            ...
            report.ManagedHostDetected = AppContext.GetData("TRUSTED_PLATFORM_ASSEMBLIES") is not null;
            report.IsDynamicCodeSupported = RuntimeFeature.IsDynamicCodeSupported;
            report.IsDynamicCodeCompiled = RuntimeFeature.IsDynamicCodeCompiled;
            report.NativeAot = !report.ManagedHostDetected && !report.IsDynamicCodeSupported && !report.IsDynamicCodeCompiled;
            if (options.SelfTest) { RequireNativeAot(options, report); SelfTest(); return 0; }
            ...
            try
            {
                ...
                RequireNativeAot(options, report);
    ...
    private static void RequireNativeAot(Options options, BenchmarkReport report)
    {
        if (options.RequireAot && !report.NativeAot)
            throw new InvalidOperationException("--require-aot requires a Native AOT executable without a managed host and with dynamic code disabled.");
    }
