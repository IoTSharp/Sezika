...
| 3 决策闭环 | 🚧 进行中 | `ModernBertDecisionEngine` 使用真实 marker 序列、两层 head、温度和总 token/deadline/head 预算；raw logits 与 `PrimitiveAlignment` 契约、4 个中英文固定输入 fixture、session/workspace/resident budget 已实现；真实模型 Choice/Score/Boolean typed smoke 通过（84 tokens） | 真实模型质量报告和正式 CLI 仍是后续独立证据 |
