...
状态：2026-09-25 已实现工具代码并迁移到 S3-09/10 共用的 `PromptSequenceBuilder`，画像报告升级至 schema v2。本页说明采集口径；实际构建、测试和运行状态以本轮执行证据及 [ROADMAP](../ROADMAP.md) 为准。代码迁移本身不产生 benchmark 数字或长输入性能结论。
...
真实请求仍使用其 `length_policy`，当前固定输入默认 `strict`。只要兼容方案需要截断，strict runtime 必须拒绝；工具保留发现请求的真实拒绝。将来显式兼容请求可以执行拟保留序列，但必须连同截断诊断报告，不能把原始长度冒充实际推理长度。
...
| working set / lifetime peak / CUDA snapshots | 每行前后 RSS/working-set 与进程生命周期高水位；CUDA owned peak 为 context 生命周期累计，并非该行独有，free/total 属于全设备；不包含完整 Driver/context 内存开销。 |
